...
class UserService {
  ...
  private reservationRepository: Repository<Reservation>; // Repositorio para la entidad Reservation
  ...
  async getReservations(uuid: string): Promise<Reservation[]> {
    const user = await this.getOne(uuid);
    return user.reservations; // Obtiene las reservaciones asociadas al usuario
  }
}
